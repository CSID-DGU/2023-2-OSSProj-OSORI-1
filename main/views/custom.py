...
def a_search(request):
    ...
    s_num = request.POST['subject_num']
    s_num = str(s_num)
    # 학수번호를 lecture 테이블에서 검색
    al = Lecture.objects.filter(subject_num=s_num)
    # 존재한다면 
    if al.exists():
...
def f_add_custom(request):
    ...
    if request.POST['arr_delete']:
        del_ug = UserLecture.objects.filter(student_id=user_id, subject_num__in = request.POST['arr_delete'].split(','))
        del_ug.delete()
    # 2. 추가된게 있을 경우
    if request.POST['arr_year']:
        ...
        classification_ge = request.POST['arr_classification_ge'].split(',')
        subject_credit = request.POST['arr_subject_credit'].split(',')
        # 커스텀 과목을 한행씩 UserLecture 테이블에 추가
        for row in zip(year, semester, subject_num, subject_name, classification, classification_ge, subject_credit):
            new_ug = UserLecture()
            ...
            new_ug.classification_ge = row[5]
            new_ug.subject_credit = row[6]
            new_ug.save()
    # 3. 모든 변경 후 정보변경 + 재검사
    update_json(user_id)
